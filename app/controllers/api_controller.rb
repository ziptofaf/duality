...
respond_to :json
 ...
 def disconnect
  @response = {'status'=>'failure'} and return unless logs_params[:username] && logs_params[:kilobytes_sent] && logs_params[:kilobytes_received] &&
  logs_params[:server_id] && logs_params[:api_key] && logs_params[:session_length]
  @response = {'status'=>'failure'} and return unless logs_params[:api_key]=='0mfd1INmx86TAzY3U25O'
  @response = {'status'=>'failure'} and logger.fatal "VPN with that username (#{logs_params[:username]}) SHOULDN'T EXIST. Security breach?" and return unless Account.exists?(:login => logs_params[:username])
