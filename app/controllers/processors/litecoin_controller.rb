class Processors::LitecoinController < ApplicationController
...
ipn = "http://dualitynetwork.com/processors/listen/litecoin"
processor_id=(Processor.find_by name: "litecoin").id
  error_message and redirect_to root_path and return unless (flash[:amount] && flash[:type]=="litecoin")
    @url = "placeholder" and @type="litecoin" and @amount=(flash[:amount].to_s.split(',').join('.').to_f.round(2)) and return if Rails.env.development? or Rails.env.test?
    json_hash=create_a_payment_url(flash[:amount], "litecoin", ipn)
